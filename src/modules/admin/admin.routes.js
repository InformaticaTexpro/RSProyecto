 ...
 * Migrado desde: src/routes/admin.js
 */
...
router.put('/usuarios/:id', async (req, res) => {
  ...
  try {
    await db.pool.query(
      `UPDATE usuario SET activo = ?, tipo_vendedor = ?, is_admin = ?, cod_vendedor = ? WHERE id = ?`,
...
router.post('/usuarios/:id/toggle-activo', async (req, res) => {
  try {
    ...
    await db.pool.query('UPDATE usuario SET activo = ? WHERE id = ?', [nuevoEstado, req.params.id]);
