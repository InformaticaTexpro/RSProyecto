...
/**
 * modules/ventas/ventas.routes.js
 *
 * Migrado desde: src/routes/ventas.js
      ...
      .query(`
        SELECT
          ...
          CASE
            ...
            THEN ROUND((1 - SUM(m.TotLinea) / NULLIF(SUM(m.CantFacturada * (${precioListaRealCASE})), 0)) * 100, 2)
            ELSE 0
          END AS pctDescuento
        FROM [PRODIN].[softland].[iw_gsaen] enc
        INNER JOIN [PRODIN].[softland].[iw_gmovi] m ON m.NroInt = enc.NroInt AND m.Tipo = enc.Tipo
